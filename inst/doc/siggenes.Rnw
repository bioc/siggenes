...
situation. Since SAM and EBAM have been developed for high-dimensional data,
...
object can be found in Schwender et al.\ (2006). This article is 
published in a special issue of RNews which can be obtained, e.g., by
...
is computed during the actual analysis, whereas in EBAM it has to be determined before
the actual analysis.

In \texttt{siggenes}, functions for performing a SAM or an EBAM analysis with
moderated $t$-statistics (one-class or two-class, paired or unpaired, assuming
...
Pearson's $\chi^2$-statistic (for analyzing categorical data such as SNP data) are
...
$\chi^2$&\texttt{cat.stat}&\texttt{cat.ebam}&\hspace*{2ex}--\\
\hline
\end{tabular}
\end{table}

Note that for \texttt{find.a0} only a function for the moderated
$t$- and $F$-scores is available, since the fudge factor is not required
if Wilcoxon rank sums or Pearson's $\chi^2$-statistic is employed.
...
obtained by \texttt{args(foo)} or \texttt{?args(foo)}, where \texttt{foo} is one
...
returns the additional arguments for a SAM analysis with a $t$- or an $F$-statistic. These
...
The matrix \texttt{golub} contains the expression values of the
...
standardized version of the EBAM analysis (for details, see Efron et
...
another than the default value of \texttt{delta}, the minimum posterior
probability for a gene to be called differentially expressed, let's say
\texttt{delta} $=0.95$, use
...
genes (i.e.\ the under the null hypothesis expected number of genes with a posterior
probability larger than \texttt{delta}) is used instead of the exact number. If the
...
same results as with a combination of \texttt{find.a0} or \texttt{ebam}.
...
correspond to one of the $m$ variables, i.e.\ genes, and each column to one of the $n$ observations.
...
\begin{verbatim}
c(1.4, 2.5, -1.4, -0.8, 2, 1.1, -0.9, -1.3)
\end{verbatim}
...
of falsely called genes), and two values are smaller than -1.4 (thus, 2/2 is the one-sided expected number of
falsely called genes).

\item[\texttt{s}:] A numeric vector containing the standard deviations of the expression values. If no standard deviations
...
of the under the null hypothesis expected number of test scores larger than or
...
of the under the null hypothesis expected number of test scores smaller than or
equal to \texttt{-abs(z[i])}, \texttt{i} $=1,\ldots$, \texttt{length(z)}.
\end{description}
...
t.ebam<-function(data, cl){
    ...
    out <- rowttests(data, cl)
    z <- -out$statistic
    z.dens <- denspr(z, n.int = 139, df = 5)
    m <- length(z)
