...
non-commercial users. Commercial users \textbf{MUST} have a
...
The results of the SAM analyses might differ between the
Excel version and \texttt{siggenes}. These differences are due
to the fact that these two versions of SAM have been implemented
...
Note that all these functions have many arguments. However, most of these arguments can 
typically be ignored, as they are only provided for very special purposes (e.g., for
trying to reproduce the results of an application of the Excel SAM version), and their defaults
typically work well. For \texttt{sam}, e.g., it is only necessary to know what the arguments
\texttt{data}, \texttt{cl}, \texttt{method} mean. For \texttt{d.stat}, the most important
arguments are again \texttt{data} and \texttt{cl} (see Section \ref{required}). 
Additionally, \texttt{B}, \texttt{var.equal},
and \texttt{rand} might also be important. All the other arguments should only be changed
if their meaning is completely understood, as their defaults have been chosen sensibly.
