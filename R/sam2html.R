sam2html<-function(sam.out,delta,filename,addStats=TRUE,addPlot=TRUE,addGenes=TRUE,
	...
	if(addStats){
		...
		cat(paste("<li>Prior Probability: p0 = ",mat.fdr[,"p0"],sep=""),
			...
			"<ul type=\"disc\">",
			paste("<li>Number of Significant",ifelse(isSNP,"SNPs:","Genes:"),
				mat.fdr[,"Called"]),
			paste("<li>Number of Falsely Called",ifelse(isSNP,"SNPs:","Genes:"),
				mat.fdr[,"False"]),
	...
	if(addGenes){
		...
		cat("<p><font color=",bg.col," size=2> HALLO</font></p>","\n",sep="",
			file=outfile)
		cat("<h3 align=center>",ifelse(isSNP,"Identified SNPs ", 
			"Genes Called Differentially Expressed "),"(Using Delta = ",
			mat.fdr[,"Delta"],")</h3>","\n",sep="",file=outfile)
