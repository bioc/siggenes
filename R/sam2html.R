sam2html<-function(sam.out,delta,filename,addStats=TRUE,addPlot=TRUE,addGenes=TRUE,
		...
		plotArgs=plotArguments(),bg.plot.adjust=FALSE,plotname=make.plotname(),
	...
	mat.fdr<-round(sum.out@mat.fdr,n.digits)
	mat.sig<-sum.out@mat.sig
	if(!is.null(mat.sig))
		mat.sig<-round(mat.sig,n.digits)
	else{
		if(addGenes){
			addGenes<-FALSE
	...
	if(addStats){
		...
		cat(paste("<li>Prior Probability: p0 = ",mat.fdr[,"p0"],sep=""),
			...
			paste("<li>False Discovery Rate: ",100*mat.fdr[,"FDR"],"%",sep=""),
			"</ul>","</ul>",sep="\n",file=outfile)
	}
	if(addPlot){
