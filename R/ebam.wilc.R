...
ebam.wilc<-function(data,x,y,paired=FALSE,delta=.9,p0=NA,stable.p0=FALSE,use.offset=TRUE,use.weights=TRUE,ties.rand=TRUE,
	...
	if(length(ns.df)>1){
		...
		for(i in 1:length(ns.df))
			vec.aic[i]<-AIC(glm(count ~ ns(W,ns.df[i])+offset(log(f.null)),family=poisson))
		ns.df<-ns.df[which(vec.aic==min(vec.aic))]
		cat("ns.df:",ns.df,"\n")
		}
	offset.value<-if(use.offset) log(f.null) else rep(0,length(count))
	glm.out<-glm(count/n.genes~ns(W,ns.df)+offset(offset.value),family=poisson)  # a poisson regression with natural splines
																	             # and offset is used
	f.x<-glm.out$fitted   # to estimate the density of the observed Ws
	...
	plot(W,p1,type="b",xlab="Wilcoxon score",ylab="Pr(gene significant|W score)",main="A posteriori probabilities")
	...
	fdr<-p0*false/nsig  # FDR
	cat("Number of signifcant genes:",nsig,"\n")  # some output
	ebam.out<-NULL
	if(nsig>0){
		...
		if(!is.na(file.out)){  # store some output in a file
			...
			write.table(ebam.out,file=file.out,sep="\t",append=TRUE)
			cat("\n","\n","\n","Genes called significant:","\n","\n",file=file.out,append=TRUE)
			write.table(ebam.output,file=file.out,sep="\t",append=TRUE,row.names=FALSE,col.names=TRUE,quote=FALSE)
