link.siggenes<-function(object,delta,filename,gene.names=NULL,addDataFrame=TRUE,
		...
		which.refseq="NM",chipname="",cdfname=NULL,refsnp=NULL,max.associated=2,
		n.digits=3,title=NULL,bg.col="white",text.col="black",link.col="blue",
	...
	link.genes(genenames,filename,entrez=entrez,refseq=refseq,symbol=symbol,omim=omim,ug=ug,
		...
		refsnp=refsnp,max.associated=max.associated,dataframe=dataframe,title=title,
		bg.col=bg.col,text.col=text.col,link.col=link.col,tableborder=tableborder,
		new.window=new.window,load=load)
}
