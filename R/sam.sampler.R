...
# x: the number of cases (i.e. length of 'case' in sam())
# y: the number of controls ( i.e. length of 'control' in sam())
...
sam.sampler<-function(x,y,B,paired=FALSE,balanced=FALSE,rand=NA,file.out=NA){
    if(!is.na(rand))
        set.seed(rand)
    if(!paired){
        if(!balanced){
            mat.samp<-matrix(0,B,x+y)
            for(i in 1:B)
                mat.samp[i,]<-sample(c(rep(1,x),rep(2,y)),x+y)
        }
        if(balanced){
            mat.samp.x<-matrix(0,B,x)
            mat.samp.y<-matrix(0,B,y)
            for(i in 1:B){
                mat.samp.x[i,]<-sample(rep(c(1,2),ceiling(x/2)),x)
                mat.samp.y[i,]<-sample(rep(c(1,2),ceiling(y/2)),y)
            }
    ...
    if(paired){
        ...
        mat.samp<-matrix(0,B,x)
        for(i in 1:B)
            mat.samp[i,]<-sample(c(-1,1),x,replace=TRUE)
    }
    if(!is.na(file.out))
