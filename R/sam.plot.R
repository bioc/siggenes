...
sam.plot<-function(sam.out,delta,data,q.values=TRUE,R.fold=TRUE,na.rm=FALSE,pty.square=TRUE,file.out=NA,
    ...
    if(!wilc){
        ...
        if(q.values)
            q.value<-q.value.wilc(d,p0,length(sam.out$x),length(sam.out$y),paired=sam.out$paired,rand=rand)$q.value
    ...
    if(!is.na(file.out)){  # output is stored in a file
        ...
        cat("Results of",which.sam,"using delta =",round(delta,4),"\n","\n","\n","cutlow:",round(vec.fdr[6],3),"\n","cutup:",
            ...
            "\n","\n",file=file.out)
        write.table(sam.output,file=file.out,sep="\t",append=TRUE,row.names=FALSE,col.names=TRUE,quote=FALSE)
        cat("Output is stored in",file.out,"\n")
    }
    par(pty="m")    
    invisible(return(vec.fdr,sam.output,row.sig.genes,sig.genes))
}
