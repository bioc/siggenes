...
sam<-function(data,x,y=NULL,paired=FALSE,mat.samp=NULL,B=100,balanced=FALSE,na.rm=FALSE,s0=NA,alpha.s0=seq(0,1,.05),include.s0=TRUE,
    ...
    d.bar<-apply(d.perm,1,mean)   # expected sorted d-values
    if(is.na(p0))
    ...
    if(is.null(FDR))
        FDR<-fdr.ngenes
    invisible(return(d,d.sort,s,d.bar,d.perm,mat.samp,s0,FDR,p0,fdr.ngenes,delta.ngenes,med.fdr,x,y,paired,var.0.genes))
}
