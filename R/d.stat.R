...
d.stat<-function(data,cl,var.equal=FALSE,B=100,s0=NA,s.alpha=seq(0,1,0.05),include.zero=TRUE,
		mat.samp=NULL,B.more=0.1,B.max=50000,med=FALSE,n.subset=10,gene.names=NULL,
		R.fold=1,R.unlog=TRUE,na.replace=TRUE,na.method="mean",rand=NA){
	data<-as.matrix(data)
	mode(data)<-"numeric"
	n.genes<-nrow(data)
	...
	if(R.fold>0){
		...
		if(R.fold!=1)
			msg<-c(msg,paste("Number of genes having a fold change >=",R.fold,"or <=",
	...
	msg<-c(msg,paste("Number of permutations:",nrow(mat.samp),
		...
		paste(ifelse(med,"MEDIAN","MEAN"),"number of falsely called genes is computed.\n\n"))
