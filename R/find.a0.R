`find.a0` <-
function(data,cl,fun=z.find,B=100,delta=0.9,quan.a0=(0:5)/5,include.zero=TRUE,
	...
	out<-FUN(data,cl,B=B,...)
	tmp.names<-names(out)
	if(any(!c("r","s","z.fun","mat.samp","z.norm")%in%tmp.names))
		stop("The function specified by fun must return a list consisting of\n",
			"objects called r, s, z.fun, mat.samp, and z.norm.")
	r<-out$r
	s<-out$s
	z.fun<-out$z.fun
