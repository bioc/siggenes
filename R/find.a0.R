...
find.a0<-function(data,x,y,paired=FALSE,mat.samp=NULL,B=100,balanced=FALSE,na.rm=FALSE,delta=0.9,alpha=(0:9)/10,include.0=TRUE,p0=NA,stable=TRUE,number.int=139,rand=NA,plot.legend=TRUE){
    ...
    plot(mat.post[which(mat.post[,"a0"]==vec.a0[1]),"center"],logit.post[which(mat.post[,"a0"]==vec.a0[1])],
        ...
        ylab="logit(posterior)",type="l",xlim=c(-4,4),ylim=c(0,max(logit.post)+0.5))
    ...
    for(i in 1:length(alpha)){     # "nice" names for the a0 are made
        vec.a0.name[i]<-paste(c("alpha=",alpha[i]),collapse="")
        vec.a0.names[i]<-paste(c("alpha=",alpha[i]," (",sig.a0[ifelse(include.0,i+1,i)],")"),collapse="")
    }
    if(include.0){
        vec.a0.name<-c("a0=0",vec.a0.names)
